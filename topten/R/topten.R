# You can learn more about package authoring with RStudio at:
#
#   http://r-pkgs.had.co.nz/
#
# Some useful keyboard shortcuts for package authoring:
#
#   Install Package:           'Ctrl + Shift + B'
#   Check Package:             'Ctrl + Shift + E'
#   Test Package:              'Ctrl + Shift + T'

# Roxygen documentation:

#' Building a Model with Top Ten Features
#'
#' This function develops a prediction algorithm based on the top 10 features
#' in 'x' that are most predictive of 'y'.
#'
#'  @param x a n by p matrix of n observations and p predictors
#'  @param y a vector of length n representing the response
#'  @return a vector of coefficients from the final fitted model with top 10 features
#'  @author Luke Coughlin
#'  @details
#'  This function runs a univariate regression of y on each predictor in x and
#'  calculates a p-value indicating the significance of the association.
#'  The final set of 10 predictors is taken from the 10 smallest p-values.
#'  @seealso \code{lm}
#'  @export
#'  @importFrom stats lm

topten <- function(x, y) {
  p <- ncol(x)
  if(p <10) {
    stop("there are less than 10 predictors.")
  }
  pvalues <- numeric(p)
  for(i in seq_len(p)) {
    fit <- lm(y ~ x[, i])
    summ <- summary(fit)
    pvalues[i] <- summ$coefficients[2, 4]
  }
  ord <- order(pvalues) #smallest to largest
  ord <- ord[1:10] #Smallest 10 p values
  x10 <- x[, ord]
  fit <- lm(y ~ x10)
  coef(fit)
}

#' Prediction with Top Ten Features
#'
#' This function take a set of coefficients produced by the \code{topten}
#' function and ameks a prediction for each fo the values provided in the
#' input predictors matrix.
#'
#' @param predictors a n by 10 matrix containing n new observation
#' @param coefs a vector of coefficients obtained from the \code{topten} function
#' @return a numeric vector containing the predicted values
#' @export
# Nothing to export

predict10 <- function(predictors, coefs) {
  #Reassign for convience
  X <- predictors
  b <- coefs
  X <- cbind(1, X)
  drop(X %*% b) #Vector of predicted responses
}




