...
## Course Project 2  
* My project is in **[a presentation of GitHub Pages](https://phiprime.github.io/plotlyPresentation/index.html#1)**.  
* The code can **[be found on GitHub](https://github.com/PhiPrime/plotlyPresentation)**.  
...
* `swirl` is an R package that turns the R console into an interactive learning environment.  
  + It has been used for learning in previous courses  
* These lessons will look at designing a course.  


### Creating a Lesson (1.2)  
* The package `swirlify` is needed to create swirl courses  
* `swirlify` is aware of the current working directory  

#### Set Up  
* A new course can be started with the `new_lesson` function  
```{r eval = FALSE, echo = TRUE}
library(swirlify)
new_lesson("Lesson 1", "My First Course")
```
  + After executing this will open a `yaml` file, a markup language that swirl lessons are written in.  
  + Four files are created after executing:  
  1) `customTests.R` - Specifies functions for testing the correctness of swirl questions  
  2) `dependson.txt` - List packages that are on CRAN that swirl will make sure the user has installed before the lesson will begin.  
  3) `initLesson.R` - An R script that is executed before a lesson starts. Used for things such as loading data or creating functions.  
  4) `lesson.yaml` - The yaml file that is pulled up in R Studio after creating a lesson. Where the course questions are defined.  

* My actual first lesson is located in the current directory.  

#### Adding Questions  
* The first "question" generated in the yaml file is a *meta question* which the user never sees but can provide some information about the lesson.  
* Swirlify has functions that can be executed to add the proper notation the yaml file for various question types:  
  + `wq_message` - creates a message that just requires the user to acknowledge they read it (typically with a return but other input may be valid)  
  + `wq_command` - prompts the user to enter R code into the console.  
    - CorrectAnswer can be an expression (`2 + 2`) or just the value (`4`).  
    - AnswerTests uses `omnitest` to validate the response given  
    - Hint is displayed if the user enters an incorrect answer  
  + There are various other `wq_...` commands available to generate the proper formatting in the yaml file.  
  
* Whenever a lesson is added to a course it should be added to the *course manifest* which can be done with the `add_to_manifest` function.  
  + The manifest keeps track of the order lessons should be displayed in and, in turn, what the lessons in the course are. 

#### Testing  

* A lesson can be tested with the `test_lesson` function.  
  + `swirlify` has some checks it runs on the lesson when this is executed and warnings will be shown now if some check fails.  
* `demo_lesson` is used to work through the lesson as if you are the user.  

### Some More Question Types (1.3)  
* To check what lesson & course `swirlify` is working on execute the `get_current_lesson` function.  

* We'll be looking at more advanced question types in a new lesson which can be created with the `new_lesson` command and using the same course name as seen below.  
```{r eval=FALSE, echo = TRUE}
new_lesson("Lesson 2", "My_First_Course")
```

* Other question types are:  
  + `wq_multiple` - for multiple choice questions  
  + `wq_figure` - displays a figure that is created from a script in the directory. `FigureType` indicates if the code should build on an existing figure or make a new one (either `add` or `new`).  
